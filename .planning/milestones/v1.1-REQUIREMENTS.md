# Requirements Archive: v1.1 Platform Admin API
...
- [x] **LEAD-01**: A Contacts-form submission on `apps/web` is persisted as a Lead via the API
- [x] **LEAD-02**: A Demo-form submission on `apps/web` is persisted as a Lead via the API
...
- [x] **CMS-02**: `apps/web`'s Blog list/detail pages render real Blog posts from the API, replacing `modules/blog/_data.ts`
- [x] **CMS-03**: PlatformAdmin can create, edit, and delete Pricing plans
- [x] **CMS-04**: `apps/web`'s Prices page renders real Pricing plans from the API, replacing hardcoded data and collapsing the `pricing-cards.tsx`/`comparison-table.tsx` duplication
